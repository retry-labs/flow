...
//   5. The web component (`<flow-diagram>`) exposes the same imperative
...
const bundle = fs.readFileSync(
  path.join(__dirname, '..', 'dist', 'flow-diagram.standalone.js'),
  'utf8',
);
...
check('<flow-diagram> web component exposes imperative play/pause/step API', () => {
  const el = window.document.createElement('flow-diagram');
...
Promise.resolve().then(async () => {
  ...
  console.log('OK: player API works in vanilla mount() and the <flow-diagram> web component');
});
