...
export function mount(target, opts = {}) {
  ...
  if (!container || container.nodeType !== 1) {
    throw new Error(
      typeof target === 'string'
        ? `rl-flow mount(): no element matched selector "${target}"`
        : 'rl-flow mount(): target must be a DOM element or CSS selector'
    );
  }
  ...
  if (container.__rlflowViewport) {
    try { container.__rlflowViewport.destroy(); } catch (_) { /* swallow */ }
  }
  injectStyles();
    ...
    if (hasPlayer()) {
      ...
      if (isAdvanced()) {
        ...
        state.steps.forEach((_, i) => {
          ...
          d.addEventListener('click', (e) => { e.stopPropagation(); api.gotoStep(i); });
        ...
        speedBtns = [0.5, 1, 2, 4].map((v) => {
          ...
          b.addEventListener('click', (e) => { e.stopPropagation(); api.setSpeed(v); });
  ...
  function applySpeedUI() {
    for (const { value, el } of speedBtns) {
      el.classList.toggle('fd-on', value === state.speed);
    }
  }

  function tickProgress() {
    if (!progressEl) { state._progressRAF = null; return; }
    if (!state.autoplay) {
    ...
    state._progressRAF = (typeof requestAnimationFrame !== 'undefined')
  ...
  function render() {
    ...
    try {
      ...
      stage.innerHTML = '<div style="padding:16px;color:#c0392b;font-family:monospace;font-size:12px">rl-flow render error: ' +
  ...
  const api = {
    ...
    get playerMode() { return state.playerMode; },
    setZoom(z) {
