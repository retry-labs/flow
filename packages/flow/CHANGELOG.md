...
All notable changes to **@retry-labs/flow** are documented here.
...
- **Step player**: when `graph.steps` exists, the viewport auto-renders a
  ...
  `<rl-flow>` web component exposes the same imperative methods and
  dispatches a `step-change` `CustomEvent`.
- **React wrapper player support**: `<RLFlow autoplay interval onStepChange />`
...
- **Standalone bundle** (`dist/flow.standalone.js`) now exposes
  the full library API on `window.RLFlow`, including `mount`,
  `renderSVG`, `parseDSL`, `graphToDSL`, `resolveGraph`, `SVG_STYLES`,
  and the shape helpers.
- **`<rl-flow>` web component** now uses the new `mount()` viewport
